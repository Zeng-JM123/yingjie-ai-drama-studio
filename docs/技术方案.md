...
本仓库提供的是产品体验原型与 GitHub Pages 部署：用前端模拟 Agent 状态、资产选择、分镜编辑、渲染队列、声音试听和交付。它不保存用户数据，也不调用真实模型。

生产开发时应先保留界面中的结构化交互，再逐项替换为上述 API 和事件流；不要把供应商 SDK、模型密钥或长任务调度直接放进浏览器。
